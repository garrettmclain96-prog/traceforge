const RECOVERY_SOURCES = [
  ...
  {id:'unclaimed',icon:'$',title:'State unclaimed property',group:'Money',trust:'official directory',description:'Bank accounts, insurance proceeds, checks and other property transferred to state custody.',url:'https://unclaimed.org/search/',secondary:'https://www.missingmoney.com/',secondaryLabel:'Multi-state search',input:'Check every state where you have lived or done business. MissingMoney covers many states at once.'},
...
function wireRecovery(){
  ...
  if(input){
    ...
    input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();loadRecoverySweep();}});
  }
