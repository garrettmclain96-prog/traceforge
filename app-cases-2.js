function exportCase(c=currentCase()){
  if(!c)return toast('Open a case first.',true); const blob=new Blob([JSON.stringify(c,null,2)],{type:'application/json'}); const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`traceforge-${c.name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'case'}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
}
...
function commitEvidenceImport(){ const c=currentCase(); if(!c||!state.importPreview)return; for(const x of state.importPreview.items){const ev={id:uid('ev'),...x,retrievedAt:nowIso(),provenance:'imported-user-supplied',provider:'Import'};c.evidence.push(ev);ensureEntity(c,ev.subject,'entity',[ev.id]);}const n=state.importPreview.items.length;state.importPreview=null;touchCase(c);render();toast(`${n} imported evidence item${n===1?'':'s'} saved.`); }
